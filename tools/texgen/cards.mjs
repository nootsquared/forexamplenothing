...
// The gamble wheel: eight wedges wearing the rarity metals — mostly common
// steel, a slice of blue, a sliver of purple, one golden promise
export const WHEEL_S = 96;
const WEDGES = ['common', 'rare', 'common', 'epic', 'common', 'rare', 'common', 'legend'];
export function generateWheel() {
  const { canvas, ctx } = makeCanvas(WHEEL_S, WHEEL_S);
  const c = WHEEL_S / 2 - 0.5;
  const rng = mulberry32(9);
  for (let y = 0; y < WHEEL_S; y++) {
    for (let x = 0; x < WHEEL_S; x++) {
      const dx = x - c;
      const dy = y - c;
      const d = Math.hypot(dx, dy);
      if (d > 46) continue;
      let color;
      if (d > 44) color = '#20242e';                        // rim
      else if (d > 42) color = '#4a5160';
      else if (d < 7) color = '#20242e';                    // hub
      else if (d < 9) color = '#f0c552';                    // hub ring
      else {
        const angle = Math.atan2(dy, dx) + Math.PI;         // 0..2π
        const wedge = Math.floor((angle / (Math.PI * 2)) * 8) % 8;
        const within = ((angle / (Math.PI * 2)) * 8) % 1;
        const [bright, mid, dark] = METALS[WEDGES[wedge]];
        if (within < 0.03 || within > 0.97) color = '#12161f'; // spoke lines
        else color = d < 26 ? mid : within < 0.5 ? mid : dark;
        if (d > 39 && within > 0.44 && within < 0.56) color = bright; // rim stud per wedge
      }
      ctx.fillStyle = color;
      ctx.fillRect(x, y, 1, 1);
    }
  }
  // speckle the steel so it reads as metal, not plastic
  for (let i = 0; i < 220; i++) {
    const a = rng() * Math.PI * 2;
    const d = 10 + rng() * 32;
    ctx.fillStyle = 'rgba(255,255,255,0.06)';
    ctx.fillRect(Math.round(c + Math.cos(a) * d), Math.round(c + Math.sin(a) * d), 1, 1);
  }
  return canvas;
}

// The pin the prize stops under
export function generateWheelPointer() {
  const { canvas, ctx } = makeCanvas(12, 14);
  for (let y = 0; y < 12; y++) {
    const half = Math.max(0, 5 - Math.floor(y / 1.4));
    ctx.fillStyle = y < 3 ? '#fff3c4' : '#f0c552';
    ctx.fillRect(6 - half, y, half * 2, 1);
  }
  ctx.fillStyle = '#8a5c16';
